...
prg = "c:/Users/unhoc/Documents/copilot_pair/11_bottles_of_beer/bottles.py"
...
def test_float():
    ...
    rv, out = getstatusoutput(f"{interpreter} {prg} --num {bad}")
    assert rv != 0
...
def test_one():
    ...
    rv, out = getstatusoutput(f"{interpreter} {prg} --num 1")
    assert rv == 0
    assert out == expected
...
def test_two():
    ...
    rv, out = getstatusoutput(f"{interpreter} {prg} -n 2")
    assert rv == 0
    assert out == expected
...
def test_random():
    ...
    for n in random.choices(list(sums.keys()), k=10):
        ...
        rv, out = getstatusoutput(f"{interpreter} {prg} {flag} {n}")
