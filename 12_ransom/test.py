...
prg = "c:/Users/unhoc/Documents/copilot_pair/12_ransom/ransom.py"
...
def test_text1():
    ...
    for seed, expected in tests:
        cmd = f'{interpreter} {prg} {seed_flag()} {seed} "{in_text}"'
        print(cmd)
...
def test_file1():
    ...
    for seed, expected in tests:
        rv, out = getstatusoutput(f"{interpreter} {prg} {seed_flag()} {seed} {fox}")
        assert rv == 0
        assert out.strip() == expected
...
def test_file2():
    ...
    for seed, expected in tests:
        rv, out = getstatusoutput(f"{interpreter} {prg} {seed_flag()} {seed} {now}")
        assert rv == 0
        assert out.strip() == expected
