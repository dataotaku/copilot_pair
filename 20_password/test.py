...
prg = "c:/Users/unhoc/Documents/copilot_pair/20_password/password.py"

words = "c:/Users/unhoc/Documents/copilot_pair/inputs/words.txt"
...
def test_bad_file():
    ...
    bad = random_string()
    rv, out = getstatusoutput(f"{interpreter} {prg} {bad}")
    assert rv != 0
...
def test_bad_num():
    ...
    rv, out = getstatusoutput(f"{interpreter} {prg} {flag} {bad} {words}")
    assert rv != 0
...
def test_defaults():
    ...
    rv, out = getstatusoutput(f"{interpreter} {prg} -s 1 {words}")
    assert rv == 0
    assert out.strip() == "\n".join(
...
def test_num():
    ...
    rv, out = getstatusoutput(f"{interpreter} {prg} -s 1 -n 1 {words}")
    assert rv == 0
...
def test_num_words():
    ...
    rv, out = getstatusoutput(f"{interpreter} {prg} -s 1 -w 2 {words}")
    assert rv == 0
...
def test_min_word_len():
    ...
    rv, out = getstatusoutput(f"{interpreter} {prg} -s 1 -m 5 {words}")
    assert rv == 0
    assert out.strip() == "\n".join(
...
def test_max_word_len():
    ...
    rv, out = getstatusoutput(f"{interpreter} {prg} -s 1 -x 10 {words}")
    assert rv == 0
    assert out.strip() == "\n".join(
        [
...
def test_l33t():
    ...
    rv, out = getstatusoutput(f"{interpreter} {prg} -s 1 -l {words}")
    assert rv == 0
    assert out.strip() == "\n".join(
