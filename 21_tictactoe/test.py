...
prg = "c:/Users/unhoc/Documents/copilot_pair/21_tictactoe/tictactoe.py"
...
""".strip()
    ...
    rv, out = getstatusoutput(f"{interpreter} {prg}")
    assert rv == 0
...
def test_bad_board():
    ...
    for bad in ["ABC", "...XXX", "XXXOOOXX"]:
        rv, out = getstatusoutput(f"{interpreter} {prg} --board {bad}")
        assert rv != 0
...
def test_bad_player():
    ...
    rv, out = getstatusoutput(f"{interpreter} {prg} -p {bad}")
    assert rv != 0
...
def test_bad_cell_int():
    ...
    for bad in [0, 10]:
        rv, out = getstatusoutput(f"{interpreter} {prg} --cell {bad}")
        assert rv != 0
...
def test_both_player_and_cell():
    ...
    rv, out = getstatusoutput(f"{interpreter} {prg} --player {player}")
    assert rv != 0
...
""".strip()
    ...
    rv, out = getstatusoutput(f"{interpreter} {prg} -b .........")
    assert rv == 0
    assert out.strip() == board
...
""".strip()
    ...
    rv, out = getstatusoutput(f"{interpreter} {prg} --board ...OXX...")
    assert rv == 0
    assert out.strip() == board
...
""".strip()
    ...
    rv, out = getstatusoutput(f"{interpreter} {prg} -b ......... --player X -c 1")
    assert rv == 0
    assert out.strip() == board
...
""".strip()
    ...
    rv, out = getstatusoutput(f"{interpreter} {prg} --board XXO...OOX --p O -c 5")
    assert rv == 0
    assert out.strip() == board
...
def test_mutate_cell_taken():
    ...
    rv1, out1 = getstatusoutput(f"{interpreter} {prg} -b XXO...OOX --player X --cell 9")
    assert rv1 != 0
    assert re.search('--cell "9" already taken', out1)

    rv2, out2 = getstatusoutput(f"{interpreter} {prg} --board XXO...OOX --p O -c 1")
    assert rv2 != 0
...
def test_winning():
    ...
    for player in "XO":
        ...
        for board in wins:
            ...
            out = getoutput(f"{interpreter} {prg} -b {test_board}").splitlines()
...
def test_losing():
    ...
    for i in range(10):
        ...
        out = getoutput(f'{interpreter} {prg} -b {"".join(losing_board)}').splitlines()
